...
使用方法:
    # 查看帮助
    python convert_human_data_20d.py --help
    
    # 转换数据，减速倍数为2
    python convert_human_data_20d.py --input ./human/slow --output ./processed --slow_factor 2.0
    
    # 转换数据，不进行减速处理
    python convert_human_data_20d.py --input ./human/high --output ./processed --slow_factor 1.0
    
示例:
    python convert_human_data_20d.py \
      --input /media/testuser/data_department2/vr_robot/human/white_desk \
      --output /home/testuser/code/opensource/human-policy/data/recordings/processed/human_white_desk \
      --slow_factor 1.0
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Convert human data from 16D to 20D format',
    ...
    parser.add_argument('--slow_factor', '-f',
                        type=float,
                        required=True,
                        help='减速倍数 (例如: 2.0 表示2倍减速, 1.0 表示不减速)')
    ...
    for h5_file in h5_files:
        ...
        try:
            convert_human_file(h5_file, output_file, slow_factor=slow_factor)
            episode_idx += 1
        except Exception as e:
